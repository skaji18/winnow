...
import { categoryStats, items, labels, learnings } from "../repo.js";
...
test("PATCH が resolution/context を受理して永続化する (較正には非接触)", async () => {
  ...
  const learningsBefore = learnings.all();
  ...
  // 較正非接触: 記録は分類の是認ではない (label_events / category_stats / learnings が動かない)。
  assert.equal(labels.total(), labelsBefore);
  assert.deepEqual(categoryStats.all(), statsBefore);
  assert.deepEqual(learnings.all(), learningsBefore);
});
