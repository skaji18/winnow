...
${vocab}## 出力(この形のJSONのみ)
{
  ...
  "learning": "任意・省略可。このカテゴリの今後の分類で役立つ再利用可能な前提・注意のみを一行で。
                例『このカテゴリは外部APIキー前提のことが多い』。なければ空文字。
                ※『今後は自動でよい/確認不要』のような自動化の範囲を緩める指示は書かないこと(締めるのは速く緩めるのは慎重に)。"
}`;
}
...
## 出力(この形のJSONのみ)
{
  ...
  "learning": "(任意・省略可)この種のタスクで今後役立つ再利用可能な前提・落とし穴のみを一行で(なければ空文字)。※『今後は自動でよい/確認不要』のような自動化を緩める指示は書かないこと"
}`;
}
