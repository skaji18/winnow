...
const HEADING_SUPPLEMENT = "## 人間からの補足情報・指示";
...
test("instruction なし: 追加指示の見出しは一切出ない(後方互換)", () => {
  ...
  assert.ok(!p.includes(HEADING_SUPPLEMENT));
});
...
test("成果あり(hasPriorOutcome)+instruction: 従来の方向修正の見出しで注入する", () => {
  const p = executePrompt(makeItem(), "", "もっと簡潔に\n表形式で", false, "", {
    hasPriorOutcome: true,
  });
  assert.ok(p.includes(HEADING_REDIRECT));
...
test("成果なし+instruction: 中立の補足情報として注入し『前回の成果物』とは言わない", () => {
  // ゲート由来 proposed の承認に人間が補足を添えた等 = worker は一度も走っていない。
  const p = executePrompt(makeItem(), "", "この方針でOK。◯◯は対象外。", false, "", {
    humanApproved: true,
  });
  assert.ok(p.includes(HEADING_SUPPLEMENT));
  assert.ok(p.includes("この方針でOK。◯◯は対象外。"));
  assert.ok(!p.includes("前回の成果物を踏まえ"), "存在しない成果物を前提にしない");
  assert.ok(!p.includes(HEADING_REDIRECT));
});

test("reviewMaterial 分岐は hasPriorOutcome より優先される(レビュー再走でも前提として読める)", () => {
  const p = executePrompt(
    makeItem({ reviewOfId: "src-1" }),
    "",
    "◯◯の観点も見て",
