...
`src/server` 内で `process.env` から読み取られるのは次の 6 つだけ（`NODE_ENV` は同一オリジン保証層と起動時の公開構成ゲートが読む。後述）。
...
| `WINNOW_HOST` | `127.0.0.1` | バインドホスト。リバースプロキシ同居構成では変更不要。非 loopback は起動時警告＋`NODE_ENV=production` 必須 | `config.ts:26`, `index.ts:151-166` |
| `WINNOW_ALLOWED_HOSTS` | （空） | カンマ区切りの追加許可ホスト名（公開ホスト名）。Origin/Host 許可リストに合流。設定時は `/mcp` が loopback Host 限定になり、`NODE_ENV=production` 必須 | `config.ts:29-32`, `security.ts:25`, `security.ts:126-133` |
| `WINNOW_ALLOWED_PORTS` | （空） | カンマ区切りの追加許可ポート。HTTPS(443) 公開では不要（Origin にポートが乗らない） | `config.ts:35-38`, `security.ts:26` |
| `NODE_ENV` | （未設定=dev 扱い） | `production` 以外で dev 分岐（Vite origin 許可＋シークレット免除）。公開構成 env と dev の併用は起動拒否 | `security.ts:16`, `index.ts:150-158` |
...
- **REST にも MCP にも認証は作らない（ユーザ識別子を持たない）。** トークン・API キー・セッションチェックは存在しない。代わりに `src/server/security.ts` の**同一オリジン保証層**が、loopback バインドに加えて次を `/api`・`/mcp` 全体に `onRequest` フック（`registerSecurityHook`, `index.ts:53`）で掛ける。
  - **(a) Origin/Host 許可リスト**（DNS rebinding / 他オリジン誘導の遮断）。許可ホストは `127.0.0.1` / `localhost` / `[::1]` / `::1`（`security.ts:19`）＋ `WINNOW_ALLOWED_HOSTS` の追加分（`security.ts:25`。リバースプロキシ公開用 → [§1](#1-環境変数)）。許可ポートは本サーバの `SERVER_PORT`（空ポート=不問）、dev（`NODE_ENV !== "production"`）では Vite の `5174` も追加（`security.ts:21-22`）、`WINNOW_ALLOWED_PORTS` の追加分も合流（`security.ts:26`）。**Host ヘッダ欠落は deny に倒す。** 不許可なら `403 {error:"origin not allowed"}`（`security.ts:115-118`）。
  - **(b) ローカルシークレット**: サーバ起動毎に `randomBytes(24).toString("hex")` で生成（`LOCAL_SECRET`, `security.ts:13`）し `index.html` の `window.__WINNOW_SECRET__` に注入。ブラウザは `X-Winnow-Secret` ヘッダで送る。**状態変更系（`/api` の非 GET）はこのシークレットを要求**し、欠落・不一致なら `403 {error:"missing local secret"}`（`security.ts:120-123`）。dev（`IS_DEV`）ではシークレットを免除（`security.ts:79`）。`/mcp` はローカル claude の正規経路なのでシークレット免除（Origin/Host 検証のみ）。ただし `WINNOW_ALLOWED_HOSTS` 設定時（=公開構成）は `/mcp` を loopback Host 限定にする（`403 {error:"mcp is loopback-only"}`, `security.ts:126-133`）。`/healthz`・`/ws`・静的アセットは対象外（`security.ts:113`）。
