...
| `claudeControlCmd` | `"claude --permission-mode acceptEdits"` | ✅ | `z.string()`（さらに `claudeAllowedFlags` 許可リスト検証, 後述） |
| `claudeWorkerCmd` | `"claude --permission-mode acceptEdits"` | ✅ | `z.string()`（さらに `claudeAllowedFlags` 許可リスト検証, 後述） |
