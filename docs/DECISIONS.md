...
- **決定**:
  ...
  - **状態機械 — 非接触+終端保護1点**: 新 executionStatus・新 LabelAction・新ゲート・下流の自動
    ...
    **status='done' も rejected と同様にスキップ**する（job は決着させる。reconcile 経路で
    executionStatus='running' のまま跨いだ場合のみ実行軸を timed_out へ降格 — 経緯と根拠は
    後続節「human handoff バッチのレビュー修正」）。timed_out を人間が手で
  ...
  - **DB・API・UI 差分**: db.ts CODE_SCHEMA_VERSION 4→5 + migrateV4toV5（ensureColumn
    ...
    （context.ts が import する単一真実源。queue.ts の hasDownstreamSiblings は受け手判定
    （未完×下流）の意図的な別式で共有しない）。queue.ts に hasDownstreamSiblings の
...
## human handoff バッチのレビュー修正（終端保護の全経路化・done の畳み・cross_repo の in-flight 例外）

- **問い**: 前節の終端保護は遅着 sentinel 経路（sweep/reconcile）だけに入り、(a) reconcile の
  done/rejected スキップが executionStatus='running' を永久残留させ（inFlightCount の幽霊
  in-flight が点火予算・自己更新ガード・/healthz を恒久汚染。done を解いても sweep は timed_out
  限定走査で回収経路なし）、(b) 最頻の通常完了経路（runExecution→applyExecuteResult）は人間の
  done+resolution を依然全面上書きし、(c) queue は done×timed_out/failed を再浮上カードとして
  永久表示し（rejected で塞いだ穴と同型）、(d) cross_repo ガードは実走中 (running) の兄弟まで
  status done/rejected で完了側に数え、(e) TreeView の楽観ロックは 3秒ポーリングがトークンだけ
  最新化し陳腐化した defaultValue との組で他所の編集を 409 なしで巻き戻す。
- **中核**: 終端保護（人間の処分が勝つ）を worker 応答の item 反映**全経路**へ拡張し、その際
  status/resolution/成果列には触れず **executionStatus='running' のみ timed_out へ降格**する。
  timed_out なら in-flight に数えられず、done/rejected を解いた瞬間に既存の sweepLateExecutions
  が sentinel から回収する＝「done を解けば回収される」が全経路で自己整合する。
- **決定**:
  - **終端保護の単一真実源**: executor.ts に `isHumanFinalized`（status done/rejected）と
    `settleHumanFinalized`（running のみ timed_out へ降格・他は非接触）を新設し、
    applyExecuteResult 先頭・runExecution 失敗系 3分岐・reconcileOnBoot・sweepLateExecutions が
    共有する。全 skip 時にレビュー leaf 生成・extractLearning も発火させない（worker 生出力は
    jobs.output に残る＝成果は失われない）。「running のまま skip だけ」は不可 —
    requestExecution の二重着火ガードが running を早期 return し再実行も塞がる袋小路になる。
    既に決着済み（running 以外）の実行軸は timed_out へ巻き戻さない。
  - **queue の done 畳みは 2.5（autoDone 取消ハンドルの後・failed/timed_out 再浮上の前）**:
    done×timed_out/failed は「止まった項目の再浮上」カード（「完了すれば自動取り込み」文言と
    再実行ボタン＝人間が完了済みのタスクへの再実行導線）にしない。rejected と同じ 1.2 への
    単純拡張は不可 — applyExecuteResult は成功時 status='done' を書くため、autoDone 取消ハンドル
    （§4-4: autoExecuted×succeeded×未受領）の対象は正に status='done' で、先に畳むと取消の手が
    消える。undo/事後編集で done を解けば従来どおり再浮上する。
  - **cross_repo の完了側除外は「物理的に走っていない場合」に限定**: done/rejected でも
    executionStatus が running/queued の兄弟は pending 側に残す（実行中に board で done にしても
    worker は他 repo を現に変更し続けている）。timed_out は in-flight 例外に**含めない** —
    done/rejected×timed_out は sweep が skip して永久に解けず、前バッチが閉じた「完了済み兄弟が
    別 repo leaf を永久に塞ぐ」穴が再発する（走行追跡は sentinel 回収に委ねる現行設計と整合）。
  - **TreeView InlineItemText の楽観ロックは基準版スナップショット**: expectedUpdatedAt は
    「表示中の本文（defaultValue）が基づく版」＝mount 時に useRef へ固定した
    {value, updatedAt} を送り、保存成功時にサーバ応答で更新する。無変更ガード（v===saved）の
    比較対象も基準版（最新 saved と比べると他所更新後の無入力 blur が逆発火する）。409 後は
    409 応答の current で基準版を最新化しつつ、ユーザが明示的に再編集（onInput）するまで
    再送しない＝保護を一発で剥がさない。
- **却下した代替**: textarea を item.updatedAt で key して remount（無関係フィールドの更新でも
  updatedAt が繰り上がり書きかけ入力が消える＝「409/失敗時の入力保持」不変条件に抵触）/
  reconcile スキップでの executionStatus='failed' 倒し（queue 再浮上文言が「実行失敗」になり、
  遅着 sentinel 回収対象＝timed_out の語義から外れる）/ done 畳みの 1.2 への単純拡張（autoDone
  取消ハンドル消失）/ cross_repo in-flight 例外への timed_out 追加（永久ブロック穴の再発）。
- **defer**: QueueCard「完了にする」の buildDonePatch も最新ポーリングの item.updatedAt を
  トークンに使う同型構造だが、draft は state 管理・resolution 初期値が常に空の完了経路で
  窓が構造的に狭い（クリック時点との差≦3秒）ため基準版 ref 化は実測後 / done×timed_out の
  横断ボード上の表示強調（実行軸の残留を人間に見せる面）。
